...
import gc
from data import init_embedding_matrix,makedata
import datetime
...
XP_FOLDER = bool_dir(rootpathdata + 'model')
...
def BIRNN_ATTENTION_LAYERS(nb_words, word_embedding_matrix, nepochs=50, batch_size=1024, shuffle=True, trainable=False,
                           ...
                           model_weights_file=os.path.join(XP_FOLDER, 'trial.h5'), load=False,
...
def BILSTM_WORDEMBEDDING(nb_words, word_embedding_matrix, nepochs=50, batch_size=1024, shuffle=True, trainable=False,
    ...
    num_dense = 400  # np.random.randint(100, 150)
    rate_drop_lstm = 0.2
    rate_drop_dense = 0.2
    ...
    m = Dropout(rate_drop_dense)(m)
    m = BatchNormalization()(m)
    m = Dense(int(num_dense/2), activation='relu')(m)
    m = Dropout(rate_drop_dense)(m)
    m = BatchNormalization()(m)
...
if __name__ == '__main__':
    ...
    model_weights_file = os.path.join(XP_FOLDER,
